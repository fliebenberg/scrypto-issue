...
use utils::parent::change_main_dapp_metadata;

pub mod utils;

#[test]
fn set_metadata_test() {
    ...
    let metadata = env
        .test_runner
        .get_metadata(
            GlobalAddress::try_from(env.parent_dapp_def).unwrap(),
            "icon_url",
        )
        .unwrap();
    println!("old metadata: {:?}", metadata);
    change_main_dapp_metadata(&mut env);
    let metadata = env
        .test_runner
        .get_metadata(
            GlobalAddress::try_from(env.parent_dapp_def).unwrap(),
