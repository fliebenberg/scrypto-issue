...
#[test]
fn setup_env_test() {
    let mut env = utils::setup_test_env(0, false, dec!("1000000"), dec!("0"), true);
    println!(
...
fn immediate_claim_test() {
    ...
    utils::token::show_token_state(&env.token1_component, &mut env.test_runner);
    let token1_balance = env.test_runner.get_component_balance(
        env.owner_account.address.clone(),
        env.token1_address.clone(),
    );
    let owner_badge_balance = env.test_runner.get_component_balance(
        env.owner_account.address.clone(),
        env.token1_owner_address.clone(),
    );
    println!("Token1 balance: {}", token1_balance);
    println!("Owner badge balance: {}", owner_badge_balance);

    let _first_claim_receipt = utils::txs::claim_first_buy_tokens(
        &env.token1_component,
        &env.token1_owner_address,
        &env.owner_account,
        &mut env.test_runner,
    );
    // println!("Claim receipt: {:?}", _first_sell_receipt);
    // println!("Token state after sell:");
    utils::token::show_token_state(&env.token1_component, &mut env.test_runner);
